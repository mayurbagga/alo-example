...
import { AbiComponent, AbiItem, ContractAbi } from "@/app/types";
...
export const getEventValues = (
  ...
  if (!event) {
    console.error(`Event with name ${eventName} not found in ABI.`);
    return null;
  }
  ...
  const log = logs.find(
    (log) =>
      log.topics && log.topics[0]?.toLowerCase() === eventTopic.toLowerCase()
  );

  if (!log) {
    console.error(`Log for event ${eventName} not found in receipt.`);
    return null;
  }
  ...
  if (!topics || !Array.isArray(topics) || !data) {
    console.error(`Invalid log structure for event ${eventName}.`);
    return null;
  }
